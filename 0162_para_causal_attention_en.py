...
FORWARD_DIR = Path("outputs/qwen15_18b_para_v2_00")
#SAVE_DIR = Path("outputs/try/causal_attention/attention_weights_en")
SAVE_DIR = Path("outputs/try/causal_attention/attention_weights_en_para")
...
def find_token_indices_for_word(prompt: str, target_word: str, component: str):
    ...
    if not all_occurrences:
        ...
        if len(all_occurrences) < 2:
            print(f"[⚠] Warning: Only one occurrence of '{target_word}' found, but expected two.")
...
def find_causal_block(prompt: str, labels: dict, lang: str = "zh"):
    """找到因果关系块"""
    block_split = {}
    info_question_list = prompt.split("\n")
    print(f"info_question_list: {info_question_list}")
    info = info_question_list[3]
    question = info_question_list[4]
    if lang == "en":
        causal_block_list = info.split(', ')
        for block in causal_block_list:
            if labels["cause_subject"] in block:
                block_split["cause"] = block
                if "." == block[-1]:
                    block_split["cause"] = block[:-1]
            elif labels["intermediate_subject"] in block:
                block_split["effect1"] = block
            elif labels["final_subject"] in block:
                block_split["effect2"] = block
                if "." == block[-1]:
                    block_split["effect2"] = block[:-1]
    elif lang == "zh":
        causal_block_list = info.split('，')
        for block in causal_block_list:
            if labels["cause_subject"] in block:
    ...
    print(f"block_split: {block_split}")
    return block_split
...
def compute_component_attention(attentions, token_indices, num_heads):
    ...
    for layer_idx, layer_attention in enumerate(attentions):
        for head_idx in range(num_heads):
            ...
            # avg_ratio = sum(component_ratios) if component_ratios else 0.0
...
def load_theme_data(theme_name: str):
    ...
    for raw_sample, forward_sample in zip(raw_data, forward_data):
        if raw_sample["id"] != forward_sample["id"]:
            print(f"Warning: ID mismatch between raw_sample and forward_sample: {raw_sample['id']} != {forward_sample['id']}")
            continue
        raw_sample["is_correct_en"] = forward_sample.get("is_correct_en", False)
        raw_sample["input_ids_en"] = forward_sample.get("input_ids_en", None)
        raw_sample["attentions_en"] = forward_sample.get("attentions_en", None)
...
def process_sample(model, sample: dict):
    ...
    tokens_out = tokenizer.convert_ids_to_tokens(input_ids[0])

    labels = sample["labels"]["en"]
    # 添加标记词组件
    labels["connective_once"] = "Once"
    labels["connective_then"] = "then"
    labels["connective_if"] = "if"
    labels["connective_therefore"] = "Therefore"
    labels["connective_final_result"] = "final result"
    labels["question_subject"] = labels["cause_subject"]
    labels["question_verb"] = labels["cause_verb"]
                                     
    # 获取attention权重
    attentions_en = sample["attentions_en"]
    if attentions_en is None:
    ...
    # 获取attention head的数量
    num_heads = attentions_en[0].shape[0]  # 使用第一个attention层的head数量
    ...
    block_split = find_causal_block(prompt, labels, lang="en")

    for component in components:
        value = block_split[component]
        ...
        if not token_indices:
            print(f"Warning: Could not find tokens for {component}: {value}")
            continue

        attention_sums = compute_component_attention(attentions_en, token_indices, num_heads)
    ...
    return {
        ...
        "prompt": prompt,
        "labels": labels,
        "is_correct_en": sample.get("is_correct_en", False)
    }
    ...
    for sample in tqdm.tqdm(theme_data):
        try:
            ...
            if result is not None:
                if result["is_correct_en"]:
                    print(f"Correct sample: {result['sample_id']}")
                    correct_results.append(result)
                else:
                    print(f"Incorrect sample: {result['sample_id']}")
